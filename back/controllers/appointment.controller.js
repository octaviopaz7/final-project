...
export const getAppointments = async (req, res) => {
  try {
    ...
    if (phonesArray.length > 0) {
      ...
      appointments = await appointmentsService.getAppointmentsByPhones(
        phonesArray
      );
    } else {
...
export const getAppointmentByPhone = async (req, res) => {
  try {
    ...
    const filteredAppointments = appointments.filter(
      (appointment) => appointment.phone === req.params.phone
    );
    if (filteredAppointments.length > 0) {
      res.status(200).json(filteredAppointments); // Retorna la cita si existe
    } else {
      res.status(404).json({
        message: "No se encontró un turno pendiente para ese teléfono",
...
export const cancelAppointmentByNumber = async (req, res) => {
  try {
    ...
    const appointment = await appointmentsService.getAppointmentByPhone(
      req.params.phone
    );
    if (!appointment) {
    ...
    const updatedAppointment =
      await appointmentsService.updateStatusAppointment(
        appointment.id,
        "Cancelado"
      );
...
export const deleteAppointment = async (req, res) => {
  try {
    const appointment = await appointmentsService.deleteAppointment(
      req.params.id
    );
...
export const getAppointmentsByDate = async (req, res) => {
  try {
    ...
    console.log(`Ruta alcanzada con la fecha: ${date}`); // Confirmar que la ruta es alcanzada
    ...
    if (appointments && appointments.length > 0) {
      ...
      res
        .status(404)
        .json({ message: "No se encontraron turnos en esta fecha" });
    }
  } catch (error) {
