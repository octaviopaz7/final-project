...
const AddForm = ({ fetchAppointments }) => {
    ...
    const schema = yup.object().shape({
        ...
        dateAppointment: yup.date().required('Fecha de cita es requerida'),
    });

    const handleSubmit = async (values, { resetForm }) => {
        const appointmentData = {
            ...values,
            dateAppointment: combineDateAndTime(selectedDate, selectedTime),
        };

        try {
            await axios.post('http://localhost:3000/api/appointments', appointmentData, { withCredentials: true });
            resetForm();
            fetchAppointments();
        } catch (error) {
                        ...
                        <Col md="4" className="form-calendar-col d-flex flex-column">
                            ...
                            <DatePicker
                                ...
                                onChange={date => {
                                    ...
                                    setFieldValue('dateAppointment', date); 
                                }}
